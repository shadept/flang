...
- **Stdlib style lives in `stdlib/README.md`.** Read it before editing anything under `stdlib/`: file layout and declaration order, `self` naming, allocator-last, the `clone`/`deinit`/`retain` vocabulary, and the idioms (iterators over index loops, fields over pointer arithmetic).
