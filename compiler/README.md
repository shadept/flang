...
```
flang build           # run from inside compiler/
```

Invokes the stage-0 compiler, resolves dependencies declared in
`flang.toml`, and emits `build/flang[.exe]`.

From the repo root, `build.cs` drives the whole chain:

```
dotnet run build.cs                 # stage 1 -> dist/<rid>/flang
dotnet run build.cs -- --stage2     # + stage 1 compiles the compiler again
dotnet run build.cs -- --stage3     # + stage 3, and check the fixpoint
```

The builder is `dist/<rid>/flang` when it exists, else the committed seed
at `boot/<rid>/flang-seed`. A clean clone has neither and starts from the
seed — see [`boot/README.md`](../boot/README.md).
...
round-trips every compiler source file byte-identical. `cst_explorer`
emits a JSON dump of source + tokens + CST + AST + diagnostics, consumed
by `cst_explorer_web` for visualization.

`flang build` runs end-to-end: it resolves the project, type-checks
...
lowering, project builds). The `compiler` project is just the CLI shell
