...
- **Import order.** The leading import block is regrouped - `core`, `std`,
  everything else (dependencies, declared or not), then the project's own
  modules by the manifest's name - one blank line between groups, alphabetical
  by dotted path within a group so a package façade sits directly above its
  children, `pub import` sorted in with the rest. An own-line comment above an
  import travels with it; a trailing comment is part of its line; exact
  duplicates collapse to the first. The block ends at the first line that is
  not an import, a comment or blank, so an import inside a decl-level `#if`
  stays put. Runs on the text before parsing (`src/imports.f`), so the token
  check sees the sorted source as its input. Redundant imports (a module also
  reached through a façade) are not the formatter's business: a formatter
  never changes what a file imports.
...
| `sort-imports` | `true` | regroup and sort the leading import block |
...
**Compiler layout.** `dotnet run build.cs` builds `compiler/` and installs the result as `dist/<rid>/flang`. The script first rebuilds the cold-start seed `boot/<rid>/flang-seed` whenever any `.c` beside it is newer (a checkout that brings in a promote does exactly that), then builds with the installed compiler unless the seed is newer than it — the seed rule lets the sources use whatever the seed supports, so an older builder is not trusted with them. A stale seed does not fail loudly: it miscompiles into a binary that segfaults on everything. Installing a *copy* into `dist/` is what stops a self-build from overwriting the binary running it: `flang build` in `compiler/` writes `compiler/build/flang`, never the one in `dist/`.
