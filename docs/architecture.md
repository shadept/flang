...
**The checker outlives one check.** `AnalyzedProject` owns the `Checker`, and `checker.f::begin_demand` readies it for the next demand. What carries is everything registered by a stable key: the nominal registry whole (`nominal_registry.f::carried_copy` - declared and generated names WITH their resolved bodies, plus the anonymous-record definitions and the map that interns them), the alias bodies, and each module's cached name-collection and body-resolution diagnostics. Closure environments are rebuilt by every body pass, so `check_all` retires their ids into `recycled` on the way out and the next pass's identically-ordered lambdas reclaim them. Everything else a check computes names variables of the engine that inferred it, and that engine goes.
