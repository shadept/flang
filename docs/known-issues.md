...
**Update 2026-08-23 (RFC-021 phase 1):** the reference compiler no longer writes sidecars as a side effect — expansion is in memory and `.generated.f` appears only with `--emit-generated`. All sidecars were deleted from the tree the same day. Until phase 4 (self-host expander) lands, the bootstrap cannot compile stdlib modules that use generators — a self-build is expected to fail, and the stage-2 = stage-3 fixpoint must be re-verified once phase 4 lands. (`--emit-generated` can regenerate sidecars for debugging, but nothing should depend on them.)
