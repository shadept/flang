...
| Parameter copy-on-write (RFC-026) | ✅ | `param_elision.f` runs per function before it reaches the module: a by-value aggregate parameter the body only reads keeps the caller's pointer, spec §3.2's promise rather than an optimization. 1514 copies removed from the self-build. `usize as &T` is E2122 (literal `0` exempt, the null-reference idiom), `&T as usize` is W2004, both suppressible with `#allow(CODE, ...)` on the enclosing declaration |
...
| Full self-build | ✅ | stage-1 (2026-08-21), then stage-2 and stage-3 (2026-08-22): every stage builds the full 99-module project. `-v` prints the per-function skip report with reasons. A skipped `main` is a build error naming the refusal chain, and skipped project functions warn without `-v` (2026-08-26) — never a bare linker failure |
...
| `#if` compile-time conditionals evaluated | ✅ | 27/10 (incl. `file.f::open_flags`, in `main`'s graph) | landed 2026-08-20: conditions parse as real FLang expressions (`parse_expression` + `stop_at_brace`, paren-free `#if cond {` canonical), evaluated strictly (`flang_parser.comptime`: E2116 unknown name, E2117 non-bool, E2118 operand misuse — reference parity); only the active branch is checked; divergence = active branch's. Decl-level flattens once post-projection (`flatten_module_decls`, active decls spliced via `Module.set_decls`). `--target-os`/`--target-arch` override the context (threaded `ResolveCtx.comptime` → `Checker.comptime`/`LowerCtx.comptime`). 2026-09-12: `#if` sees names in scope (`type_info(T).copyable` / `.kind` / `.name` on a type parameter, decided per specialization, or on a non-generic nominal; `type_of(v)` on a value) through `CtLookup.name`; `TypeInfo.copyable` joins the runtime descriptor |
...
| Global `const` declarations | ✅ | 101/17 | Each const becomes an aligned byte global. `const_bytes` classifies the initializer: a value known at compile time is encoded straight into the global's `init_bytes`, and an address it contains becomes a `fir.Reloc` the backend spells as a C struct of byte runs and pointers behind a `#define g_<name> (&g_<name>_r)`, so no reference site changes. Covered: scalar literals and unary negation, `null`, struct literals (recursively, at their layout offsets), string literals (a reloc to the interned bytes plus the length), a function name, an enum variant bare or qualified (its tag), the address of another const, and a pointer-to-pointer cast of one. Not covered, and so runtime-dependent: a call, a float literal, field access on a const receiver. The corpus is entirely in the covered set — the self-compile emits **zero** `__finit_*` functions. A runtime-dependent const keeps the M11 path: a synthesized `__finit_*` lowering the initializer, called from `main` before any user instruction, wired AFTER the drop pass, which also poisons any reader of a const whose init died (`reads … whose initializer was dropped`) — absent, never silently zero. A statically-initialized const has no init function, so `ConstInfo.init_sym` is null and the drop pass never treats its readers as reading uninitialized storage. Emission order is creation order, which puts a global ahead of any global holding its address; function forward declarations precede the globals for the same reason. Reads resolve through `RtConst(fqn)` targets the checker records on both the decl and every read |
...
| Operators dispatching to user `op_*` fns (aggregate operands) | ✅ | String `==` everywhere | M11: comparison dispatch lands (`lower_operator_binary`) with negation and `op_cmp`-derivation — the Ord tag test resolves Less/Equal/Greater from the definition and compares against their wire tags (`variant_tag`), the same values construction stores. Payload-less enum `==`/`!=` is a builtin tag compare. M12: ARITHMETIC dispatch (`op_add`/`op_sub`/`op_mul`/`op_div`/`op_mod`/bitwise/shifts) lands through the same seam |
...
| Reified `Type(T)` values (full `core.rtti.TypeInfo`) | ✅ | 21 harness tests | M12 filled the whole record — name, size, align, kind, type params / args, struct fields (name, offset, nested TypeInfo), enum variants, a function type's params + return — at the use site, depth-capped at 3. 2026-09-12: `static_typeinfo` emits one data-segment descriptor per type instead (ADR-0001), memoized before its members are built so a self-citing type links; no cap. `type_info` / `type_of` return `&TypeInfo` into that table (`type_info` intercepted at lowering like `size_of`), so `==` is type identity. `type_args` is a `Slice(&TypeInfo)`: one descriptor pointer per argument |
...
| Enum variant construction (`Some(x)`, `Color.Red`, `None`) | ✅ | ~1550/60+ | M7: tagged form builds tag-then-payload into a fresh slot, the tag being the variant's wire value (`variant_tag`: a naked enum's explicit `= N`, else the declaration index); niche `Option(&T)` is a retype (`None` = null ptr, `Some(p)` = its payload ptr). Multi-payload construction refuses (2 sites, matching the pattern side) |
...
| Casts `x as T` | ✅ | 845/75 | M11: trunc / sign-directed extension (source signedness picks zext/sext), fp↔int by the non-float side's sign, fp widths, ptr↔int, ptr↔ptr no-op, repr-compatible aggregate retypes, `[T; N] as T[]` decay (builds the `{ptr,len}` view), tagged-enum ↔ int through the I32 discriminant (a naked enum's discriminant IS its declared tag, so `Less as i32` is `-1`). `x as bool` refuses (needs C's `!= 0`, zero sites). The backend spells i64::MIN as `(-…807LL - 1)` |
| Array literals `[a, b]`, `[v; N]` | ✅ | ~52/9 | M11: stride-addressed element stores into a slot; the zero-repeat form is one memset, any other repeat is one fill loop (no count cap; aggregate values memcpy per element); a decay-coerced literal wraps the fresh array in a Slice view. `arr.len` is the constant length; `arr.ptr` the array's address. **`N` must be an integer literal** — a named module const (`[0u8; CAP]`) refuses the whole body (2026-08-25) |
