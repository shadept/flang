...
- A shift's result has the left operand's type. A shift count written as an
  integer literal must be in `0..width-1` of that type (E2121); `isize`/`usize`
  are checked against their widest (64-bit) layout. A runtime count is not
  checked and must satisfy the same bound.
...
- **Overload resolution handles same-named imports**. Two different imports may bring in functions with the same name; the type checker resolves the call by parameter types. Candidates are ranked by **implicit-conversion count** first: an argument that matches its declared parameter type as-is always beats one that had to be coerced. A `String` argument IS a `String` — it decays to `u8[]` only when no `String` parameter takes it, so `write(&File, String)` wins over a `write(&File, u8[])` sibling for a string literal. Conversion-count ties fall to **structural specificity**: each concrete type constructor in a declared parameter is a constraint the call's arguments had to satisfy, and the candidate whose supplied parameters carry more of them wins — `deinit(&List($T))` beats the universal `deinit(&$T)` fallback for a list receiver, and a concrete-receiver method like `any(&Dict($K,$V), $F)` beats an unconstrained catch-all like `any($I, $F)`. Type variables contribute nothing to specificity, and a position is only scored when its argument's type is already fully known — parameters the call left to defaults, and positions whose argument still contains an unresolved type variable (an unsuffixed literal, a half-inferred aggregate), are skipped, since an unbound argument matches any parameter shape at zero cost and cannot corroborate structure. Skipped for *scoring* is not the same as unconstrained: an unsuffixed numeric literal carries a constraint to the numeric primitives (plus `char` for an integer literal), so a candidate whose parameter is a nominal, a reference, or a tuple fails to unify and never reaches ranking at all. Remaining ties fall to lower total cost (the UFCS receiver adaptation, omitted defaulted parameters), then fewer quantified type parameters, then literal-preference arbitration, and finally declaration order. A UFCS receiver's adapted value ↔ `&T` form competes inside the same ranked candidate set (its cost penalty does not count as a conversion) rather than in a fallback pass, so a catch-all matching the un-adapted receiver cannot preempt a more specific overload that needs the adaptation. Genuinely ambiguous calls (no unique strictest overload) error at the call site. (The specificity count is a tuned heuristic standing in for a real constraint system.)
- **No aliases, no selective imports, no relative paths.**
- **Auto-imported core prelude.** Every module implicitly imports [`core.prelude`](../stdlib/core/prelude.f), a curated barrel that `pub import`s the core modules (`core.option`, `core.string`, `core.panic`, `core.cmp`, etc.). All core symbols are therefore visible without an explicit import. The prelude itself is the only module exempt from the auto-import.
