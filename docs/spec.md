...
- **Default parameter values**: Parameters may have defaults: `fn foo(x: i32, y: i32 = 10)`. Default values are evaluated fresh at each call site (Kotlin semantics). Parameters with defaults must follow all required parameters.
- **Named arguments**: Call-site arguments can be named: `foo(y = 20, x = 10)`. Uses `=` syntax. Positional arguments must precede named arguments. Named arguments are not supported for indirect calls (function pointers). The type checker resolves names and reorders arguments into parameter order.
- **Variadic parameters**: A function may have a single variadic parameter as its last parameter: `fn bar(..args: i32)`. The variadic parameter is received as a slice (`i32[]`). At the call site, variadic arguments are packed into a stack-allocated array and coerced to a slice. Zero arguments produce an empty slice. Variadic parameters cannot have default values. Foreign functions use C-style `...` instead.
- All three features are **caller-side transformations**: the type checker resolves names, fills defaults, and packs variadics. Lowering and codegen see a normal, fully-populated positional argument list.
...
- **Lambda expressions** (non-capturing): Anonymous functions written as `fn(x: i32, y: i32) i32 { x + y }`. Lambdas desugar to synthesized module-level functions (`__lambda_N`) and produce a function reference. Parameter types can be omitted when inferrable from context (e.g., `let f: fn(i32) i32 = fn(x) { x + 1 }`). Lambdas may not capture variables from enclosing scopes; referencing an outer variable is an error.
