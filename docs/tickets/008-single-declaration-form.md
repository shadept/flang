...
- Stdlib (`std/`, `core/`), examples (`examples/*/src/*.f`), and harness tests are all in form 2. The only form-1 occurrences in the tree are the deliberate negative-test fixtures `tests/harness/warnings/deprecated_struct_syntax.f` and `deprecated_enum_syntax.f`, which assert E1050/E1051 fire.
...
Audit `tests/harness/` for any test fixtures using form 1; rewrite.
