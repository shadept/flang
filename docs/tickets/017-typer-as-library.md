...
Phase 1 landed — every module from the layout is implemented and the lib compiles cleanly. The RTTI codegen blocker is fixed, so the lib is consumable. The compile pipeline (lex → parse → project → `check_all`) is packaged in a new [`flang_driver`](../../lib/flang_driver/) library — `analyze(source, path) AnalyzedUnit` — so every front-end exe (`build`, `test`, the LSP) reuses one analysis path. `bootstrap build <file.f>` runs it end-to-end on a single file and reports diagnostics with `file:line:col`. Caveat: Phase 3 body inference is still a thin slice — `check_binary`/`check_call` are `fresh_var()` stubs, and the `unify` outcomes in `check_return`/`check_let`/`check_if` are discarded, so only name-resolution errors surface today (see [known-issues](../known-issues.md)). No import graph yet, so checker visibility is "current module only" — a build of a file that imports `std` floods unknown-name errors until the project driver lands. (The codegen bug that previously forced flang_typer usage into each exe's `main.f` — a tuple-layout cache collision — is now fixed; see known-issues.) Engine + coercion exercised by colocated `test {}` blocks in `inference_engine.f`, run with `flang test` (the C# compiler is the test driver until bootstrap self-hosts). The `examples/typer-smoke` smoke-check example is retired: it targeted the pre-refactor `substitution` module and no longer built.
