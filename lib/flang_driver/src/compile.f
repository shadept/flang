...
// separator styles depending on where they were built (manifest globs vs stdlib joins).
fn contains_path(paths: &List(OwnedString), path: String) bool {
    for i in 0..paths.len {
        if same_path(paths[i].as_view(), path) {
            return true
        }
    }
    return false
...
fn same_path(a: String, b: String) bool {
    ...
    for i in 0..a.len {
        const ca = if a[i] == '\\' { '/' } else { a[i] }
        const cb = if b[i] == '\\' { '/' } else { b[i] }
        if ca != cb {
            return false
        }
    }
