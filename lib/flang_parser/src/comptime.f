...
// through `ct_eval`. The environment is two layers: the closed context (`platform.os` /
// `platform.arch`, `runtime.testing` / `runtime.release`, `runtime.env["KEY"]` with Dict semantics
// - `String?`, unwrap with `??`) and, inside a template, the bindings (parameters, `#for`
// variables)
// layered on top. Template values are the `core.rtti` shapes (§5):
// `TypeInfo` / `FieldInfo` / `VariantInfo` / `ParamInfo` / `TypeKind`. Semantics mirror the
// reference compiler's CompileTimeEvaluator:
//
...
//
// Also home to `flatten_module_decls`: decl-level `#if` resolves once, right after projection -
...
// The compiling host's context. os/arch resolve through this compiler's OWN #if - evaluated when
// the compiler binary itself was built.
...
// A compile-time value. `OptStr` is what `runtime.env[key]` yields - FLang Dict indexing returns an
// optional. The `Ns*` variants are the
...
pub fn ct_eval(env: &CtEnv, e: &Expr) Result(CtValue, CtError) {
    return e.* match {
        ...
        Identifier(id) => {
            ...
            if id.name == "platform" {
                return Ok(CtValue.NsPlatform)
            }
            if id.name == "runtime" {
                return Ok(CtValue.NsRuntime)
            }
            if id.name == "TypeKind" {
        ...
        Coalesce(c) => {
            ...
            lhs match {
                ...
                _ => ct_err("E2118",
                    $"left operand of `??` must be an optional (a dict lookup), got {describe(&lhs)}",
                    c.span)
            }
        }
...
fn ct_eval_binary(env: &CtEnv, b: &BinaryExpr) Result(CtValue, CtError) {
    ...
    if li.is_some() and ri.is_some() {
        ...
        b.op match {
            ...
            Div => {
                if y == 0 {
                    return ct_err("E2118", $"division by zero", b.span)
                }
...
fn ct_expect_int(env: &CtEnv, e: &Expr) Result(i64, CtError) {
    ...
    return v match {
        I(i) => Ok(i)
        _ => Err(CtError { code = "E2118", message = $"expected an integer, got {describe(&v)}",
            span = expr_span(e) })
    }
}
...
fn ct_call(env: &CtEnv, c: &CallExpr) Result(CtValue, CtError) {
    ...
    if name == "type_named" {
        const tn: String = arg match {
            S(s) => s
            _ => return ct_err("E2118", $"`type_named` expects a string, got {describe(&arg)}",
                arg_span)
        }
    ...
    const text: String = arg match {
        S(s) => s
        Ident(i) => i
        _ => return ct_err("E2118",
            $"`{name}` expects a string or identifier, got {describe(&arg)}", arg_span)
    }
    if name == "lower" {
        return Ok(CtValue.S(lower_case(env, text)))
    }
    if name == "snake_case" {
        return Ok(CtValue.S(snake_case(env, text)))
    }
    if name == "pascal_case" {
        return Ok(CtValue.S(pascal_case(env, text)))
    }
    return ct_err("E2116", $"unknown compile-time function `{name}`", c.span)
}

fn ct_member(env: &CtEnv, recv: &CtValue, member: String, span: SourceSpan) Result(CtValue,
    CtError) {
    return recv.* match {
        NsPlatform => {
            if member == "os" {
                return Ok(CtValue.S(env.ctx.os))
            }
            if member == "arch" {
                return Ok(CtValue.S(env.ctx.arch))
            }
            ct_err("E2116", $"unknown compile-time member `{member}`", span)
        }
        NsRuntime => {
            if member == "testing" {
                return Ok(CtValue.B(env.ctx.testing))
            }
            if member == "release" {
                return Ok(CtValue.B(env.ctx.release))
            }
            if member == "env" {
                return Ok(CtValue.NsEnv)
            }
            ct_err("E2116", $"unknown compile-time member `{member}`", span)
        }
        NsTypeKind => {
            if member == "Primitive" {
                return Ok(CtValue.Kind(KIND_PRIMITIVE))
            }
            if member == "Array" {
                return Ok(CtValue.Kind(KIND_ARRAY))
            }
            if member == "Struct" {
                return Ok(CtValue.Kind(KIND_STRUCT))
            }
            if member == "Enum" {
                return Ok(CtValue.Kind(KIND_ENUM))
            }
            if member == "Function" {
                return Ok(CtValue.Kind(KIND_FUNCTION))
            }
            ct_err("E2116", $"unknown compile-time member `{member}`", span)
        }
        ...
        Field(f) => {
            ...
            if member == "type_info" {
                return Ok(CtValue.TypeInfo(type_info_of(env, f.type_expr)))
            }
            if member == "offset" {
                return ct_err("E2120",
                    $"`offset` is not available at template time (layout is computed after expansion)",
                    span)
            }
...
fn ct_type_member(env: &CtEnv, t: &CtTypeInfo, member: String, span: SourceSpan) Result(CtValue,
    CtError) {
    if member == "name" {
        return Ok(CtValue.S(t.name))
    }
    if member == "kind" {
        if t.kind < 0 {
    ...
    f match {
        Some(fnty) => {
            for i in 0..fnty.params.len {
                let pname: String = ""
                if i < fnty.param_names.len {
                    pname = fnty.param_names[i]
                }
                if pname.len == 0 {
...
pub fn is_primitive_name(name: String) bool {
    return name == "i8" or name == "i16" or name == "i32" or name == "i64" or name == "u8"
        or name == "u16" or name == "u32" or name == "u64" or name == "isize" or name == "usize"
        or name == "f32" or name == "f64" or name == "bool" or name == "char" or name == "void"
        or name == "never"
}
...
pub fn spell_type_into(sb: &StringBuilder, te: &TypeExpr) {
    te.* match {
        ...
        Reference(r) => {
            sb.append('&')
            spell_type_into(sb, r.inner)
        }
        Optional(o) => {
            spell_type_into(sb, o.inner)
            sb.append('?')
        }
        Array(a) => {
            sb.append('[')
...
// Resolves every decl-level #if in the module, immediately after projection: only the active
// branch's declarations survive into the module's decl list (nested #if resolves recursively).
// Invalid conditions produce diagnostics and drop both branches.
pub fn flatten_module_decls(m: &Module, ctx: &ComptimeCtx, diags: &List(Diagnostic),
