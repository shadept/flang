// Coercion rules.
//
// A coercion turns an `actual` type into an `expected` type by means other than direct structural
// unification - integer widening, float widening, optional wrapping, and so on. Rules are
// *directional*:
// `from → to`, mirroring `unify(actual, expected)`.
//
// A rule's `try_*` returns `Coercion { result_ty, cost, side_unifications }` or `null`. The engine
// commits the side-unifications atomically (any failure rolls everything back) and replaces
// `actual`'s entry in `InferenceResults.node_types` with `result_ty`. No coercion ever leaks past
// the type checker: lowering reads the result type from the side-table and never inspects the
// surrounding slot.
//
// Rules match shapes through the `TypeInterner` the engine passes in. Prim-on-prim rules need it
// only for the node lookup; nominal-aware rules (option wrapping, string to byte-slice, array
...
// rolls everything back if any one fails.
pub type Constraint = struct {
    a: Ty
    b: Ty
}

// A successful coercion's payload. `result_ty` is what the engine will store as the unified type.
// `cost` lets overload resolution score candidates that need fewer coercions higher.
// `side_unifications` is usually empty for simple widening; option wrapping, slice decay, and the
// nominal/anon-struct rules use it to communicate "also unify these inner types".
...
// way: unsigned `from` can widen into signed `to` only when `to`'s rank is strictly larger (so the
// unsigned range fits).
//
// `bool` widens to any integer (treating false=0 / true=1).
//
// `isize` / `usize` rank: 4 (64-bit target). When 32-bit targets land, thread the pointer width
// through the engine and parameterise here.
// ─────────────────────────────────────────────────────────────────────
...
pub fn try_integer_widening(it: &TypeInterner, from: Ty, to: Ty,
...
pub fn try_float_widening(it: &TypeInterner, from: Ty, to: Ty,
    allocator: &Allocator? = null) Coercion? {
    let pf = prim_kind_of(it, from) match { Some(p) => p, None => return null }
    let pt = prim_kind_of(it, to) match { Some(p) => p, None => return null }
...
// ─────────────────────────────────────────────────────────────────────
// Nominal-aware rules - require the `NominalRegistry` to resolve the FQNs of well-known sugar types
// (`Option`, `String`, `Slice`).
//
// Each rule's `apply` either returns a `Coercion` whose `result_ty` is the target nominal and whose
// `side_unifications` express the inner-arg constraint, or `null` to indicate "this rule does not
// fire here".
// ─────────────────────────────────────────────────────────────────────
...
pub fn try_type_to_typeinfo(it: &TypeInterner, from: Ty, to: Ty, reg: &NominalRegistry,
    allocator: &Allocator? = null) Coercion? {
...
pub fn try_string_to_byte_slice(it: &TypeInterner, from: Ty, to: Ty, reg: &NominalRegistry,
    allocator: &Allocator? = null) Coercion? {
    let fn_n = nominal_node_of(it, from) match { Some(n) => n, None => return null }
    let tn = nominal_node_of(it, to) match { Some(n) => n, None => return null }
    ...
    if it.child_ids(tn.args)[0] != prim_of(PrimitiveKind.U8) {
        return null
    }
    return Some(simple(to, allocator))
}
...
// reference engine applies its rules in both directions; this pair keeps byte views and strings
// interchangeable.
pub fn try_byte_slice_to_string(it: &TypeInterner, from: Ty, to: Ty, reg: &NominalRegistry,
    allocator: &Allocator? = null) Coercion? {
...
// Array decay rules. Four variants, distinguished by whether `from` is `Array` or `&Array` and
// whether `to` is `Slice(T)` or `&T`.
//   - `[T; N] → Slice(T)`
//   - `&[T; N] → Slice(T)`
//   - `[T; N] → &T`
//   - `&[T; N] → &T`
// Each emits one side-unification: the array's element type unifies with the target's element /
// inner type.
pub fn try_array_decay(it: &TypeInterner, from: Ty, to: Ty, reg: &NominalRegistry,
    allocator: &Allocator? = null) Coercion? {
    // Identify the array element of `from`, if any (with or without a surrounding reference).
...
fn decay_to(it: &TypeInterner, to: Ty, elem: Ty, reg: &NominalRegistry,
    allocator: &Allocator?) Coercion? {
...
fn decay_to_slice(it: &TypeInterner, to: Ty, n: &NNominalNode, elem: Ty, reg: &NominalRegistry,
    allocator: &Allocator?) Coercion? {
    let slice_id = lookup_well_known(reg, FQN_SLICE)
    if slice_id.is_none() {
        return null
    }
    if slice_id.unwrap() != n.id {
        return null
    }
    if n.args.len != 1 {
        return null
...
pub fn try_nominal_to_type(it: &TypeInterner, from: Ty, to: Ty, reg: &NominalRegistry,
    allocator: &Allocator? = null) Coercion? {
    let tn = nominal_node_of(it, to) match { Some(n) => n, None => return null }
    let type_id = lookup_well_known(reg, FQN_TYPE)
    type_id match {
    ...
    // `from` must be a concrete type - not a Type(T) itself, not a bare TypeVar (the bare-var case
    // is caught at the engine level and never reaches coercion).
