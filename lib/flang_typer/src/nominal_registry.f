...
//
// Distinct from the C# `TypeRegistry`: struct fields and enum variants have their own first-class
// shapes (`StructDef` / `EnumDef`) instead of being lumped into a `FieldsOrVariants` list
// discriminated by an enum `Kind`. Payload-less enum variants carry an empty `payloads` list - no
// `void` sentinel.
//
// Lookups return `RegLookup` instead of `Option` so the caller can distinguish "not found" from
// "found but not visible" - that distinction drives a better diagnostic ("type exists in module X
...
// One enum variant. `payloads` is empty for nullary variants. `decl_span` locates the variant's own
// declaration inside the enum body, and is `none_span()` for synthesized variants.
...
pub type EnumDef = struct {
    ...
    // Naked enums (every variant payload-less, explicit integer tags). `null` for standard
    // tagged-union enums.
...
// Result of looking up a name in the registry. `NomLookFound` returns the index; `NomLookHidden`
// means the entry exists but its defining module is outside the caller's visibility scope - used to
// emit a "did you forget `import X`?" hint. `NomLookMissing` means no entry by that name exists
// anywhere.
...
pub type NominalRegistry = struct {
    // Keyed by id, not positional. Ids come from `next_id` and are never reused, so evicting an
    // entry leaves a hole instead of renumbering every id after it - the nominal type nodes already
    // baked into other results stay valid.
    defs: Dict(NominalId, NominalDef)
    next_id: NominalId
    by_fqn: Dict(String, NominalId)
    // FQN strings live on the registry. `def.fqn` and `by_fqn` keys are views into the heap buffers
    // owned here. The buffers themselves are heap-allocated by `OwnedString` and do not move when
    // the list re-allocates, so the views stay valid for the registry's life.
...
// buffer keeps the `String` views in both `def.fqn` and `by_fqn` stable for the registry's
// lifetime. The `fqn` field on `def` is overwritten with the stable view, so the caller can pass a
// placeholder there.
//
// Caller is responsible for checking duplicates first - registering an FQN twice overwrites the
// index and leaks the previous definition.
...
// Replace the fqn field on a NominalDef with a stable view. Used by `register` to repoint def.fqn
// from a caller-temporary view to the view owned by `self.owned_fqns`.
...
// Re-register a definition under an id the registry handed out before, using the FQN view it
...
// A deep copy of every live definition, body included, at the id it holds. `next_id` carries, so
// ids stay monotone across the copy. Field and variant lists are cloned; the `Ty` handles inside
// them stay valid as long as the type table they index does. Used to hand a demand's registry to
// the next one while the previous demand's snapshot keeps its own.
...
// definition struct itself stays in place with dangling list buffers - callers overwrite or evict
// it right after.
...
// The definition at `id`. Panics on an id that was never registered or has been evicted; callers
// holding a possibly-stale id use `find`.
...
// Live definitions. Not an id bound - iterate `0..next_id` and skip the holes for that.
pub fn len(self: &NominalRegistry) usize {
    return self.defs.len()
}

// Overwrite the definition at `id` in place. The replaced value is NOT deinited: callers rebuild a
...
// the hole rather than resolving to a neighbour.
...
// FQN-only lookup, no visibility scope. Used by template expansion and downstream phases that have
// already accepted the symbol.
...
pub fn lookup(self: &NominalRegistry, name: String, vis: &Visibility) NomLookup {
    ...
    if direct.is_some() {
        return NomLookup.NomLookFound(direct.unwrap())
    }
    ...
    if vis.current_module.is_some() {
        ...
        if q.is_some() {
            return NomLookup.NomLookFound(q.unwrap())
        }
    }

    // Short-name scan with visibility filter. First visible hit wins; a hidden hit is remembered so
    // we can return it as a hint when nothing visible matches.
    ...
    for entry in self.by_fqn {
        ...
        if vis.allows(module) {
            return NomLookup.NomLookFound(id)
        }
...
// First visible enum declaring a variant `name` with exactly `arity` payloads. Registration order
// breaks ties - qualify with the enum name when two visible enums share a variant shape. ponytail:
// linear scan over every def per call; index variant names if checker profiles flag it.
pub fn lookup_variant(self: &NominalRegistry, name: String, arity: usize,
    vis: &Visibility) NominalId? {
...
// The owning module prefix of an FQN, given its `last_dot` index; empty when the name is
// unqualified.
...
pub fn enum_payloadless(self: &NominalRegistry, id: NominalId) bool {
    return self.get(id).* match {
        NomEnum(e) => {
            for i in 0..e.variants.len {
                if e.variants[i].payloads.len > 0 {
                    return false
                }
            }
            true
