...
// What one specialization owns: the key buffer, the overlay tables, and the concrete-params list.
// `decl` is a shallow copy into the module arena and the name/module strings are views - not ours.
...
pub fn register(self: &SpecializationRegistry, spec: Specialization) u32 {
    ...
    // `spec.key` was moved into `with_id.key` on construction; the OwnedString's heap buffer is
    // separate from the entry that holds it, so a later rehash does not move it and this view
    // remains valid for the registry's life.
