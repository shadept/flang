...
// appended to its ORIGIN module, so later invocations see the generated
// types and the generated declarations share the origin's FQN prefix
// and import scope. Nothing is written to disk; the driver may emit
// `<origin>.generated.f` from `emitted` when asked.
//
// Generated text is registered in the driver's file table (a padded copy
// per chunk, path `<origin>.generated.f`), so spans inside generated code
// render against the combined text the LSP and `--emit-generated` show.
...
// Where a type declaration lives: `modules[module].decls[decl]`. Indices,
// not pointers - the decl list grows as chunks are appended.
...
// Generated modules and texts outlive the checker (lowering walks the
// appended decls): the driver takes them before `deinit`.
...
// Deliberately leak the output (single-unit analysis has no slot to
// keep it alive; the appended decls reference the chunk arenas).
...
// The lookup handed to the evaluator: resolves a type name in the
// invoking module's visibility to its declaration syntax.
type ExpandCtx = struct {
    chk: &Checker
    state: &TemplateState
    modules: &List(Module)
    vis: Visibility
    // Per-invocation arena: resolved declarations are boxed here so the
    // pointers handed to the evaluator outlive the match that found them
    // (match payloads are copies).
    alloc: &Allocator
}
...
fn resolve_type_decl(raw: &u8, name: String) &TypeDecl? {
    ...
    // Through a local: indexing a `&List` struct FIELD directly is
    // miscompiled by the self-host (docs/known-issues.md, 2026-08-23).
...
// `sources[i]` / `file_paths[i]` back `modules[i]` (the driver's file
// table); generated chunks are appended to both so their file ids
// resolve. Runs after every module's nominal names are collected.
pub fn expand_templates(chk: &Checker, state: &TemplateState, modules: &List(Module), paths: &List(String),
        sources: &List(OwnedString), file_paths: &List(OwnedString)) {
    // Callers without a file table (typer unit tests) can't host template
    // bodies; nothing to expand.
    if sources.len < modules.len { return }
    ...
    loop {
        ...
        for item in work {
            ...
            const outcome = expand_one(chk, state, modules, paths, sources, file_paths, item, &parked)
            outcome match {
...
// Register a module's (or chunk's) generator definitions, type syntax and
// invocations. `first_decl` is where this module's new decls start in
// `modules[m].decls` (0 for an original module, the pre-append length for
// a chunk); `decl_base` offsets the DeclRef indices the same way.
fn index_module(state: &TemplateState, module: &Module, m: usize, decl_base: usize, path: String, source: String,
        work: &List(WorkItem), generation: u32) {
    let j: usize = 0
    for &d in module.decls {
        d.* match {
            ...
            GenInvoke(inv) => { work.push(WorkItem { module = m, inv = inv, generation = generation }) },
            Type(td) => {
...
fn expand_one(chk: &Checker, state: &TemplateState, modules: &List(Module), paths: &List(String),
        sources: &List(OwnedString), file_paths: &List(OwnedString),
        item: WorkItem, parked: &List(WorkItem)) Outcome {
    const inv = item.inv
    ...
    let ectx: ExpandCtx = .{ chk = chk, state = state, modules = modules, vis = current_visibility(chk), alloc = &a }
    const lookup: CtLookup = .{ ctx = &ectx as &u8, resolve = resolve_type_decl }
    let env = ct_env(&chk.comptime, &a, lookup)

    // Bind parameters. A `Type` argument whose nominal is not collected yet
    // parks the invocation (another expansion may produce it).
    ...
    expand_template(&env, &nodes, &raw) match {
        Ok(_) => {},
        Err(e) => {
            const sp: SourceSpan = .{ file_id = def.file_id, start = def.base + e.span.start, length = e.span.length }
    ...
    // Register the chunk as a padded file entry so its spans line up with
    // the combined generated text for this origin.
    ...
    if env("FLANG_DEBUG_TEMPLATES").is_some() { println(chunk.as_view()) }
...
fn bind_arg(chk: &Checker, env: &CtEnv, param: &GenParam, arg: &GenArg, gen_name: String, a: &Allocator) Result(CtValue, Outcome) {
    if param.kind == "Ident" {
        return arg.* match {
    ...
    if param.kind == "Type" {
        return arg.* match {
            // Boxed: the match payload is a copy, and CtTypeInfo retains
            // the pointer past this call (FromSyntax).
            TypeArg(te) => Ok(CtValue.TypeInfo(type_info_of(env, box(a, te)))),
            IdentArg(id) => {
                if !type_is_collected(chk, id.name) { return Err(Outcome.UnknownType) }
                const named: TypeExpr = TypeExpr.Named(NamedType { span = id.span, name = id.name, generic_args = list(0, Some(a)) })
...
fn report_unknown_types(chk: &Checker, state: &TemplateState, paths: &List(String), item: &WorkItem) {
    const found = state.defs.get(item.inv.name)
    if found.is_none() { return }
    const def: GenDefEntry = found.unwrap()
    chk.set_current_module(paths[item.module])
    for p in 0..def.params.len {
        if p >= item.inv.args.len { break }
        const param = def.params[p]
...
fn emitted_for(state: &TemplateState, allocator: &Allocator?, origin_path: String) &EmittedFile {
    for i in 0..state.emitted.len {
        if state.emitted[i].origin_path == origin_path { return &state.emitted[i] }
    }
...
// Re-indents generated text by brace depth (2 spaces per level), drops
// trailing whitespace and collapses blank-line runs to one. Template-body
// indentation is not significant. Braces are counted outside string /
// char literals and line comments.
// ponytail: text-level normalizer; a CST formatter replaces this wholesale when one exists.
    ...
    loop {
        ...
        if pos == n or text[pos] == '\n' as u8 {
            ...
            if line.len == 0 {
                ...
                let indent = depth
                if line[0] == '}' as u8 and indent > 0 { indent = indent - 1 }
...
fn count_braces(line: String) (usize, usize) {
    ...
    for c in line.as_raw_bytes() {
        if !in_string and !in_char and c == '/' as u8 and prev == '/' as u8 { break }
        if in_string {
            if c == '"' as u8 and prev != '\\' as u8 { in_string = false }
        } else if in_char {
            if c == '\'' as u8 and prev != '\\' as u8 { in_char = false }
        } else if c == '"' as u8 {
            in_string = true
        } else if c == '\'' as u8 {
            in_char = true
