// Ty - a 4-byte handle into the `TypeInterner` (RFC-024). One node is stored per distinct type, so
// equality of types IS equality of handles, copying a type copies an integer, and the interner is
// the single owner of every type's storage.
//
// The shape behind a handle is a `TyNode` (interner.f); consumers match on nodes through the table.
// This module keeps what needs no table: the handle itself, the identifier aliases, the fixed leaf
// ids, the primitive kinds and their predicates, and `Field` - the name/type pair records and
// nominal struct bodies share.
//
...
// Fixed leaf ids. Primitives follow at `TY_PRIM0` in `PrimitiveKind` declaration order (Bool ..
// Char) - `prim_of` is the mapping.
...
// ─────────────────────────────────────────────────────────────────────
// Handles - transparent aliases over plain integers so APIs read at a glance and the engine pays no
// wrapping overhead.
// ─────────────────────────────────────────────────────────────────────

// Inference-variable identifier. Allocated by the engine; opaque to callers. Two `TyVar`s with the
// same `VarId` are the same variable.
pub type VarId = u32

// Let-generalisation depth. Variables created at a deeper level than the current
// `enter_level`/`exit_level` cursor are eligible to be quantified by `generalize`.
pub type Level = u32
...
// One unification variable. The `level` is metadata used by `generalize`/`specialize`; two vars are
// the same variable iff their `id`s match, but a var NODE keys on (id, level) - `free_vars` reads
...
// The 14 FLang scalar primitives. Compared by tag; no string names flow through the type system at
// runtime. Diagnostic rendering goes through `prim_name(...)`.
...
// identity and rendering ignore it, so two records differing only in where they were written are
// the same type.
