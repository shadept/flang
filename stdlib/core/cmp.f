...
// auto-derivation of `<`, `>`, `<=`, `>=`, `==`, `!=` from `op_cmp`, this lets user types become
// orderable by defining a single `op_cmp` function.
//
// For primitive types, `<`/`>`/`==` always use the hardware compare; the `op_cmp` overloads here
// exist so that primitives can be passed as the default comparator to generic algorithms (see
// `std.sort`).
...
// Total-order comparison result. Tag values are fixed: the compiler compares the tag against 0 when
// deriving `<`/`>`/`<=`/`>=` from `op_cmp`.
pub type Ord = enum {
    Less = -1
    Equal = 0
    Greater = 1
...
// NaN is not ordered by IEEE 754. Here, any NaN input yields `Ord.Equal` because both `<` and `>`
// return false for NaN. Sorting a slice containing NaN is therefore not well-defined - filter or
// replace NaNs beforehand.
...
// Lexicographic comparison on tuples - `op_cmp` is auto-derived for `<`/`>`/`<=`/`>=`/`==`/`!=`, so
// user types containing tuples become orderable for free.
...
pub fn op_cmp(a: ($T1, $T2, $T3, $T4, $T5, $T6, $T7, $T8), b: (T1, T2, T3, T4, T5, T6, T7,
        T8)) Ord {
    const c0 = op_cmp(a.0, b.0)
    if c0 != Ord.Equal {
        return c0
