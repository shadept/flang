...
//
// This is the only module in std.io that declares `#foreign`. Everything the
// platform disagrees about is settled here and nowhere else:
//
...
//
// Callers hand in ordinary String views. Nothing here requires a caller to
// have NUL-terminated anything, which is the bug this module exists to make
// unrepresentable.
//
// Nothing here is meant for application code. `std.io.fs`, `std.io.file` and
// `std.io.dir` are the supported surface; they add no foreigns of their own.
// FLang has no module-private visibility, so that boundary is a convention -
// importing this directly is possible and unsupported.
...
// FileKind, FileInfo and FsError live in std.io.types - below this module, so
// the public io modules can name them without importing internals.

// Portable open modes. fs.c maps these to O_* / _O_*, whose numeric values
...
// Upper bound for a NUL-terminated path handed to the syscall layer. Matches
// Linux PATH_MAX; macOS (1024) and Windows (260, or 32767 for long paths) are
// both covered by taking the largest.
//
// The buffers below spell 4096 as a literal rather than using this constant:
// the self-hosted backend refuses a body whose array length is a named const
// (see docs/known-issues.md). Keep the two in step.
pub const FS_PATH_BUF_CAP: usize = 4096

// Return-code conventions shared with fs.c, where they are FS_R_* for the same
// reason they are prefixed here: <unistd.h> defines R_OK as 4.
const FS_R_OK: i32 = 0
...
fn to_c_path(path: String, buf: u8[]) bool {
    if path.len + 1 > FS_PATH_BUF_CAP { return false }
    let i: usize = 0
    while i < path.len {
        buf[i] = path[i]
        i = i + 1
...
// Calls `op` with `path` copied into a NUL-terminated stack buffer. Every
// path-taking syscall goes through here, so the copy happens in one place and
// no entry point can forget it. Generic over what `op` produces: the ones that
// only report success return `()`, the ones that hand back a handle, an fd or
// a stat return that.
fn with_c_path(path: String, op: $F) Result($T, FsError) {
    if path.len == 0 { return Err(FsError.InvalidArgument) }
    let buf = [0u8; 4096]
    if !to_c_path(path, buf) { return Err(FsError.NameTooLong) }
    return op(buf.ptr)
}

// Turns the shim's (status, out_err) pair into a Result. Paired with
// `with_c_path` this is the whole calling convention, in two functions.
//
// Every caller writes `err` through an out-pointer in the same expression that
// reads it:
//
//     check(__flang_fs_open(p, mode, &fd, &err), err)
//
// which reads the value the call just wrote. That is guaranteed, not lucky:
// FLang evaluates operands left to right (docs/spec.md 5.1.1), enforced by the
// backend rather than inherited from C. Regression tests live in
// tests/harness/eval_order/.
fn check(status: i32, err: i32) Result((), FsError) {
    if status != FS_R_OK { return Err(err as FsError) }
    return Ok(())
}
...
pub fn raw_open(path: String, mode: i32) Result(i32, FsError) {
    // The capture (`mode`) is used only in a straight-line expression, and the
    // branch is an `if` rather than `?`: a capture referenced inside a match
    // arm - which is what `?` lowers to - fails to resolve
    // (docs/known-issues.md). Non-capturing lambdas below use `?` freely.
...
//
// This lives in internal rather than in std.io.dir because two public modules
// need it: `Dir` wraps one, and `walk_dir` holds a stack of them. Routing the
// walk through std.io.dir instead would make fs and dir mutually dependent.
//
// `name` on a yielded entry is a view into `name_buf` and dies on the next
// `next()` call. Callers that accumulate entries must clone.
...
pub fn iter(self: &RawDir) &RawDir {
    // Returning `&self` (not a copy) means `for entry in d { ... }` mutates the
    // original, so `d.err()` stays meaningful after the loop.
    return self
}
...
// Replaces `to` when it exists, on every platform - the Windows shim uses
// MoveFileEx rather than the CRT rename, which refuses an existing target.
pub fn raw_rename(from: String, to: String) Result((), FsError) {
    // Two paths, so two nested copies - the inner lambda captures the outer's
    // buffer pointer.
...
pub fn raw_realpath(path: String, allocator: &Allocator? = null) Result(OwnedString, FsError) {
    // `buf.len` rather than FS_PATH_BUF_CAP, and an `if` rather than `?`: a
    // lambda can reference neither a module-level const nor a capture inside a
    // match arm (docs/known-issues.md). `allocator` is captured, so it is only
    // named in a straight-line expression.
