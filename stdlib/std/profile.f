...
// (calls, self, inclusive, per-call) to stderr, sorted by self time. `--profile-out <path>` on the
// build also writes folded stacks - one line per call path - which speedscope.app, inferno, or
// flamegraph.pl render as a flamegraph. `--profile-nodes` / `--profile-depth` resize the runtime's
// node pool and shadow stack when a report says it overflowed.
//
...
// Write the profile collected so far: the flat table to stderr, and folded stacks to the
// `--profile-out` path when the build named one. Counters keep accumulating - the exit report still
