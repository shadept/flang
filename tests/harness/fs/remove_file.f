...
// remove_file reports FileError: unlinking is something you do to a file, so the directory-shaped
// failures are not in its error type at all.
...
pub fn main() i32 {
    ...
    if !exists(path) {
        println("missing_after_write")
        return 2
    }
    print("created ")

    if remove_file(path).is_err() {
        println("remove_failed")
        return 3
    }
    if exists(path) {
        println("still_there")
    ...
    if !is_not_found(again.unwrap_err()) {
        println("wrong_error")
        return 6
    }
