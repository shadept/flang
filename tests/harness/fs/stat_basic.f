...
// stat() reports kind + size. exists/is_dir/is_file are thin wrappers. "/" is a known directory on
// POSIX; the tests that need it skip on Windows.
...
pub fn main() i32 {
    ...
    if !kind_is_dir(info.kind) {
        println("expected Dir")
        return 2
    }
    ...
    if is_file("/") {
        println("is_file-/ should be false")
        return 5
    }
