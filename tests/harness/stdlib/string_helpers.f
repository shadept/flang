...
pub fn main() i32 {
    ...
    if !is_ascii("hello-world") {
        return 30
    }
    if is_ascii("café") {
        return 31
    }
    if !eq_ignore_ascii_case("Hello", "HELLO") {
        return 32
    }
    if eq_ignore_ascii_case("hello", "world") {
        return 33
    }
    ...
    if combined.as_view() != "alpha|beta|gamma" {
        return 41
    }
    ...
    if sb.as_view() != "hello FLang" {
        return 60
    }
    sb.clear()

    sb.append_replaced("a.b.c.d", ".", "-")
    if sb.as_view() != "a-b-c-d" {
        return 61
    }
    sb.clear()
    ...
    if sb.as_view() != "red, green, blue" {
        return 62
    }
    sb.clear()

    sb.append_repeated("ab", 3)
    if sb.as_view() != "ababab" {
        return 63
    }
    sb.clear()
